...
`x_engage` 会生成回复候选、引用转发候选和独立短帖候选；`viral_280` 会生成一条 280 字符以内的短推。只给 X 链接时会通过 oEmbed 尝试抓取原推正文；如果原推只有外链，会继续尝试抓取外链页面的标题、摘要和正文段落。如果同时提供正文，X 链接只作为上下文保存。不会使用登录态，也不会自动发布。
